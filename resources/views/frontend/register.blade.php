...
<body>
  ...
  <header class="header header--active w-100">
    <nav class="navbar navbar-expand-lg">
      <div class="container">
        <a class="navbar-brand position-relative wow bounceInLeft" href="{{ route('index') }}">influencer</a>
  ...
  <section class="sub-banner position-relative header--fixer">
    <div class="container">
      <div class="row">
        <div class="section-header col-12 mb-4">
          <h1 class="section-header__title text-uppercase">{{ $registerinfo->first_title }}</h1>
        </div>
        <div class="col-12">
          {!! $registerinfo->first_description !!}
        </div>
        <div class="section-header col-12 my-4">
          <h1 class="section-header__title text-uppercase">{{ $registerinfo->secound_title }}</h1>
        </div>
        <div class="col-12">
          {!! $registerinfo->secound_description !!}  
        </div>
        <div class="section-header col-12 my-4">
          <h2 class="section-header__title text-uppercase">{{ $registerinfo->third_title }}</h2>
        </div>
        <div class="col-12" id="rules">
          {!! $registerinfo->third_description !!}
          <p>Lorem, ipsum dolor sit amet consectetur adipisicing elit. Impedit quis voluptates animi. Temporibus nihil minus maxime at atque repellendus error?</p>
          {{-- <ul class="rules-list">
            <li class="rules-item d-inline-flex mb-3">
              <span class="rules-icon d-inline-block mr-3">
        ...
        <div class="col-12">
          <a href="#!" class="primary-btn position-relative text-uppercase d-inline-block wow bounceInUp" data-toggle="modal" data-target="#influencerFormModal">
            <span class="position-relative">Influencer Register</span>
          </a>
        </div>
      </div>
    </div>
                ...
                <form class="influencer-form position-relative needs-validation " action="{{ route('become.an.influencer.store') }}" method="post"  enctype="multipart/form-data" novalidate>
                  ...
                  <div class="col-12 text-center mb-4">
                    <div class="custom-file w-auto">
                      <input type="file" class="custom-file-input position-absolute" name="profile_photo" id="customFile">
  ...
  <script>
    let rules = document.querySelector('#rules').childNodes;
    rules.forEach(myFunctions);
    function myFunctions(ulList) {
   if (ulList.tagName == 'UL' || ulList.tagName == 'OL') {
    ulList.setAttribute('class', 'rules-list');
    let me = document.querySelector(".rules-list").childNodes.forEach(function (item) {
            if (item.nodeName != '#text') {
                let after_text = item.textContent;
                item.textContent = '';
                let span =  document.createElement('span');
                span.setAttribute('clsss', 'rules-icon d-inline-block mr-3');
                span.innerHTML = '<i class="fas fa-crosshairs"></i>';
                span.style.color= "#222";
                let div =  document.createElement('span');
                div.style.paddingLeft = '15px';
                div.appendChild(document.createTextNode(after_text));
                item.setAttribute('clsss', 'rules-item d-inline-flex');
                ...
                // item.appendChild('<span class="rules-icon d-inline-block mr-3"><i class="fas fa-crosshairs"></i></span>');
