...
<head>
	...
	<title>
        @if ($offer_details->type == 1)
            Influencer Delivery Offer
        @elseif ($offer_details->type  ==2)
...
<body>
					...
					<div class="accordion" id="accordionExample">
						<div class="card wow bounceInUp" data-wow-delay="0.1s">
							<div class="card-header" id="headingOne">
                                ...
                                @if ($offer_details->type == 1)
                                    {{ route('deliveryoffers') }}
                                @elseif ($offer_details->type  ==2)
                                    {{ route('experienceoffers') }}
                                @else
                                    {{ route('dinneroffers') }}
                                @endif
                                    " class="back-btn d-inline-block rounded-sm"><i class="fas fa-chevron-left"></i></a>
							...
							<div id="collapseOne" class="collapse show" aria-labelledby="headingOne" data-parent="#accordionExample">
								<div class="card-body text-center">
									<figure class="individual-offer__image">
										<img src="{{ asset('uploads/shop_photo') }}/{{ $offer_details->thumbnail }}" alt="delivery-items" class="w-100">
									</figure>
                                ...
                                @endif
									<h2 class="individual-offer__title mb-3">An Exciting Experience With
										<span class="individual-offer__shop d-block my-2">{{ $offer_details->name }} @if ($offer_details->time_sidule!=null)({{ $offer_details->time_sidule }})
                                        @endif</span>
									</h2>
                                    <div>
                                    ...
                                    @elseif ($offer_details->type == 2)
									<a href="{{ route('experienceoffers') }}" class="primary-btn position-relative text-uppercase d-inline-block w-100 my-3 my-md-0">
										<span class="position-relative">No, thanks</span>
									</a>
                                    @elseif ($offer_details->type ==3)
									<a href="{{ route('dinneroffers') }}" class="primary-btn position-relative text-uppercase d-inline-block w-100 my-3 my-md-0">
										<span class="position-relative">No, thanks</span>
									</a>
    ...
    <script>
        let rules = document.querySelector('#rules').children[0];
            rules.setAttribute('class', 'rules-list');
